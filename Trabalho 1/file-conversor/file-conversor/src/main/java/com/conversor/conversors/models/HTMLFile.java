...
public class HTMLFile extends FileConversor implements IFileConversor {
   ...
   * @return String
   */
  String getValue(String coluna, String str, String end) {
    String valueString = "";
    int offIndex = 0;
    int startIndex = coluna.indexOf(str, offIndex) + str.length();
    int endIndex = coluna.indexOf(end, offIndex);

    while (startIndex < endIndex) {
      String value = coluna.substring(startIndex, endIndex);
      valueString += value + ",";

      offIndex = endIndex + 1;
      startIndex = coluna.indexOf(str, offIndex) + str.length();
      endIndex = coluna.indexOf(end, offIndex);
    }
    valueString += "\n";
